...
- 番外短片：蓮教學《Shift Breaker》唯一變形的保養流程，工具細節特寫。

## Persistent Elements（跨集維護）
- 角色外觀：蓮的短髮長度、戰鬥背心配色（黑灰主、黃線條點綴）、工裝褲的膝蓋護板磨痕需保持一致；汗膜亮度不超過 0.12 反射值。
- 武器狀態：
  - 劍盾形態：護手齒輪刮痕位置固定，盾面左上裂紋需延續；金屬度 0.62、粗糙度 0.28。
  - 充能巨斧形態：斧刃外緣磨損區域 15% 面積保持一致，黃白能量紋路節奏 90 BPM；變形後不得再切回劍盾。
- 場景破壞：
  - 高架骨架斷點座標固定，前景地面裂縫呈放射狀五條，後景有倒塌橋柱殘段；
  - 巨獸踩踏留下的粉塵雲需在 Beat 1–5 持續存在並在 Beat 11 逐步沉降；
  - 核心爆閃後紅光殘霧停留 2.2s，下一集若延續需帶有同樣顏色值（#ff3b3b，亮度峰值 120 nits 內）。
- 音樂動機：低鼓心跳在 80 BPM 基準，變形時上升到 96 BPM，擊殺後下降到 72 BPM；若後續集數採樣需沿用相同節奏。
- UI/字幕樣式：僅結尾標題卡使用白字黑邊、半透明 80% 黑底，字體保持無襯線；禁止於其他段落出現字幕。

## Camera & Lighting Continuity Table
- Act 1：
  - 焦段：24–28mm；機位高度：俯拍或膝高；運鏡：滑軌前推或側移，手持 0%。
  - 光源：主光 5500K 右上 50°，填光僅環境散射；陰影方向固定向左下；
  - 對焦策略：煙塵→巨獸→蓮背影，拉焦步階 0.4s。
- Act 2：
  - 焦段：35mm；機位：胸高；運鏡：平移＋微抖動 2–3%；
  - 光源：保持主光方向，護手火花作為補光點光源；
  - 對焦策略：前臂→護手→盾面裂紋，景深 FOV 控制在 0.8m 內。
- Act 3：
  - 焦段：35–40mm；機位：胸高到眼高；運鏡：跟隨甩斧弧線與推近；
  - 光源：日光直射＋能量紋路自發光 5200K；
  - 對焦策略：節點→斧刃→蓮眼神，慢動作段落維持對焦在斧刃邊緣。
- Act 4：
  - 焦段：50–75mm；機位：胸高或眼高；運鏡：定格後推近；
  - 光源：核心爆閃後快速回落到主光水平，避免過曝；
  - 對焦策略：核心碎裂→崩塌→蓮臉部，最後留在眼神光。

## Physics & PBR Deep Notes
- 重力與慣性：跳躍起飛腳跟離地需顯示腳尖推力造成碎石朝相反方向彈射；斧刃落下時帶動空氣形成可見壓縮波，受慣性拖出能量尾跡。
- 布料與髮絲：戰鬥背心材質粗糙度 0.34，受風擺動幅度控制在 4–6cm；短髮因汗水貼附，僅在跳躍段出現 1–2 根飛絲。
- 金屬與塵霧：
  - 斧刃金屬度 0.68、粗糙度 0.22，邊緣高光需遵循鏡面反射，避免過度模糊；
  - 齒輪噴汽的蒸汽粒子半徑 1.2–1.8mm，受重力 9.8m/s² 下墜並受環境風 1.5 m/s 推往畫面左；
  - 灰塵粒子密度在 20–35 個/立方米，粒徑 8–16μm，正午光下產生 Mie 散射形成淡白光暈。
- 光學行為：
  - 斧光能量線使用 Fresnel 邊緣光，折射率 1.45，邊緣比核心亮 0.25 stops；
  - 核心爆閃時 Bloom 半徑控制在 12–18px，避免吞線；
  - 漫畫格段落暫停景深與運動模糊，回到實拍後恢復原設定以維持連續性。

## Risk & Mitigation
- 風格衝突：同時啟用 Anime Fantasy 與 Light Glyph 時，需確認膚色保護開啟，避免薄荷藍覆蓋臉部；若出現偏色，優先保留 Anime Fantasy LUT。
- 運鏡跳軸：所有 Beat 的運鏡方向（右→左、下→上）需在 Storyboard 中標註；如必須跨軸，需在過場塵霧遮擋時進行 90° 旋轉以免左右互換。
- 曝光過高：核心爆閃控制峰值 120 nits，必要時在調色階段增加 0.2 ND 模擬，避免黑黃線稿被吞沒。

## Audio Sync Pointers
- Beat 1–2：採用低頻踩踏與遠處警示燈節奏，音量 -16 dBFS，位置鎖定左前方，營造巨獸逼近感。
- Beat 3–4：尾擊撞擊與盾面共振加入 80Hz LFE，衝擊後 0.3s 加入金屬回響；齒輪緊繃聲定位在右側。
- Beat 5–6：預充與節點點亮疊加高頻叮聲（7–9kHz），音量 -10 dBFS，與鏡頭繞拍節點同步；白霧釋放以寬闊立體聲鋪滿。
- Beat 7–9：能量流動低鳴以 Sidechain 壓低背景鼓點 1–2 dB，跳躍前 0.2s 加入上升滑音提醒；空氣撕裂音定位中心偏右。
- Beat 10：漫畫格衝擊使用瞬時白噪替代鼓點 0.2s，再帶入心跳式低頻；速度線抖動時加入短促 Hi-hat（1/8 拍）。
- Beat 11–12：瓦礫崩落以隨機化粒子音效散佈左右聲道，收束時低鼓逐步淡出至 -20 dBFS，標題卡段保持安靜並保留風聲尾巴。

## Additional QC Notes
- 逐幕確認 PBR 數值在渲染設定中落實，特別是斧刃金屬度與混凝土粗糙度，避免風格化過度平滑。
- 漫畫格段落需檢查速度線方向與 Beat 9 的運動方向一致，避免方向錯位造成視覺跳動。
- 標題卡字級需在 36–42pt 之間，保持 10% 邊界內縮，以防被平台 UI 遮擋。

## AGENT Self-Check
- 結構：Master、Continuity Layer、Technical Specs、Platform Layer、Negative Instructions、Changelog、Assumptions、Act→Beat→Angle（12 幕，含 timecode）、音樂/VFX、Next Episode 全數齊備。
- Physics & PBR：各 Beat 寫明重力/慣性、粒子落下、PBR 粗糙度與金屬度、光學行為（體積霧/反射/折射）並在變形與衝擊段落細述。
- Stylepacks：Anime Fantasy、Urban Switch FX、Light Glyph Anime、Shift Breaker Industrial Audio 均標註適用與限制，僅在特效段開啟光軌樣式，未混入衝突 LUT。
- Continuity：軸線保持蓮右後 120°，光源固定正午右上，Stage 時間線與蓄壓亮度分段一致，漫畫格後回復原光位與鏡頭站位。
- 自評：10/10，符合 15 秒｜12 幕要求並完整嵌入模板內容，若需調整將在回饋後更新。
