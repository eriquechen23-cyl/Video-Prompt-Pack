# 《鋼脈都市｜冰核衝線・神谷隼人 個人PV》— Master Prompt（15s｜12 幕）

專案欄位：
- project_name：鋼脈都市
- series_name/arc：鋼脈都市・冰核衝線篇
- episode_index：3
- slug：ice-core-charge-kamiya-hayato-pv_v01
- target_platform：短影音（9:16，YouTube Shorts / TikTok）
- duration_sec：15（依專案規範預設 15s，12 幕 × 1.25s）
- style_primary：Frost Gambler Look（冰藍博弈霜紋＋玻璃折射感）
- style_secondary：Light Glyph Anime Look（2D 日系線條＋霓虹 HUD 冰陣）
...
```
Act({節奏域/氣氛})
- 基調（明暗/色溫/動態）：{冷暖/柔硬光/移動速度/聲音密度}
- 轉場：{硬切/匹配/遮擋/疊化/運鏡銜接}
- 關鍵元素：{角色、道具、環境、光源}
- 物理寫實：{重力/慣性/摩擦/流體/布料/粒子行為}
- 高級質感：{PBR、膚質分層、光學、顆粒}

Beat({時間})
- 重點：{劇情關鍵}
- Blocking：{走位/姿態/鏡頭相對位置}
- Camera：{焦段/角度/對焦/運動}
- Lighting：{主次光、色溫、對比}
- Materials & Physics：{材質、粗糙度、金屬度、法線、重力、慣性、力場}
- Emotion & Performance：{肢體/表情/視線/節奏}
- Audio & Transition：{音效/音樂/轉場方法}
- Angle：{構圖/層次/運鏡}
```
...
音樂：{曲風/速度/編制/節奏點}
環境：{基底環境/動態元素/節點變化}
人聲：{VO 性別與語氣/唱詞或台詞/語言}
音效：{武器/碰撞/移動/魔法/特效}
混音：{動態範圍/LUFS/sidechain/立體聲寬度}
定位：{前景/中景/遠景/聲音移動路徑}
```
...
niagara_presets:
  ...
  - parallax_ui_slice
unity_vfx_graph:
  - LightGlyph.ArcTrail.vfx
  - LightGlyph.CelBurst.vfx
  - LightGlyph.RimRibbon.vfx
houdini_flipbooks:
  - glyph_ink_bloom
  - edge_flash
  - ring_slice
...
audio_sync:
  ...
  ui: sync_to_hi_hat
notes:
  - HUD 線條需跟隨角色視線方向，避免與臉部細節遮擋。
  - 施法時邊緣光可以 2-frame 閃回，強調手勢收束。
  - Bloom 控制在 0.52 以下，避免破壞 cel 線條。
```
...
- Execution Note：遠景霓虹與橋體採 camera projection 固定；黑霧用 VDB 192³，粒徑 20–35μm，吸收係數 0.12；冰核射擊採 Flip 粒子 140k 轉 spritesheet，冰束體積光使用 0.9 可見度並限制 Bloom 0.62；音畫同步使用 Frost Gambler Audio peak_events 觸發關鍵彈擊與爆裂。
...
- 自動檢核：
  ...
  3. 檢查宗教政治符號：無。
  4. 檢查風格衝突：Light Glyph Anime 與 Frost Gambler 均 ON，未混用其他 LUT。
  5. 檢查字幕語系：僅繁中+日文擬聲。
...
- Stylepacks 使用與衝突：Style Primary Frost Gambler + Style Secondary Light Glyph Anime，明確標註不可混用其他 LUT；Bloom/色散控制列出，無衝突。
